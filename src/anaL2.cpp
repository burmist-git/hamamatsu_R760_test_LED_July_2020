...
void anaL2::Loop0(TString nameF){
  ...
  TH1D *h1_deltaTimeConstTh[USBWCnChannels];
  ...
  h1D1Init(h1_deltaTimeConstTh,"h1_deltaTimeConstTh","delta TimeConstTh",40000,-2000.0,2000.0);
  h1D1Init(h1_timeConstTh,"h1_timeConstTh","timeConstTh",800,0.0,2500);
  h1D1Init(h1_sigRiseT,"h1_sigRiseT","sigRiseT",4000,0.0,20);
  h1D1Init(h1_sigWidth,"h1_sigWidth","sigWidth",4000,0.0,20);
  h1D1Init(h1_sigFallT,"h1_sigFallT","sigFallT",4000,0.0,20);
  ...
  TH1D *h1_deltaTimeConstTh_cut[USBWCnChannels];
  ...
  h1D1Init(h1_sigRiseT_cut,"h1_sigRiseT_cut","sigRiseT cut",4000,0.0,20);
  h1D1Init(h1_sigWidth_cut,"h1_sigWidth_cut","sigWidth cut",4000,0.0,20);
  h1D1Init(h1_sigFallT_cut,"h1_sigFallT_cut","sigFallT cut",4000,0.0,20);
  h1D1Init(h1_sigChargeInWindowVT_cut,"h1_sigChargeInWindowVT_cut","sigChargeInWindowVT_cut",10000,-1,10);
  h1D1Init(h1_deltaTimeCFD_cut,"h1_deltaTimeCFD_cut","delta TimeCFD cut",40000,-2000.0,2000.0);
  h1D1Init(h1_deltaTimeConstTh_cut,"h1_deltaTimeConstTh_cut","delta TimeConstTh cut",40000,-2000.0,2000.0);
  ...
  //Out Log file
  ofstream outlf;
  TString outLogfile = nameF;
  outLogfile +=".log";
  outlf.open(outLogfile.Data());
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    for(i = 0;i<USBWCnChannels;i++){
      ...
      deltaTimeConstTh = timeConstTh[i] - timeConstTh[0];
      h1_deltaTimeConstTh[i]->Fill(deltaTimeConstTh);
      ...
      if(amplMax[0]>0.006){
	//if(amplMax[1]>0.006){
	//if(deltaTimeConstTh > 42 && deltaTimeConstTh < 52){
	//if(deltaTimeConstTh > 149 && deltaTimeConstTh < 155){
	...
	h1_deltaTimeConstTh_cut[i]->Fill(timeConstTh[i] - timeConstTh[0]);
	if(i == 1)
	  outlf<<eventIDL1<<endl;
	//}
	//}
      }
    }
  ...
  for(i = 0;i<USBWCnChannels;i++){
    ...
    h1_deltaTimeConstTh_cut[i]->Write();
  }
